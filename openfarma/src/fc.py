...
db_beneficios = Chroma(
    persist_directory=DB_BENEFICIOS_PATH, embedding_function=embedding
)
...
db_indicaciones = Chroma(
    persist_directory=DB_INDICACIONES_PATH, embedding_function=embedding
)
db_uso = Chroma(persist_directory=DB_USO_PATH, embedding_function=embedding)
db_propiedades = Chroma(
    persist_directory=DB_PROPIEDADES_PATH, embedding_function=embedding
)
    ...
    try:
        ...
        if not df.empty:
            for _, row in df.iterrows():
                data[row.iloc[1]] = (
                    f"Stock: {row.iloc[2]}. Precio: ${row.iloc[3]}. Promoción: {row.iloc[4]}."
                )
        return data
...
def retrieveVectorDB(database: Chroma, context: str, k: int = 10) -> dict:
    """
...
def buscar_productos(**kwargs):
    ...
    default_message = (
        f"No se encontraron productos que cumplan con la consulta sobre: {problem}."
    )
...
def buscar_productos_por_presentacion(**kwargs):
    ...
    ids = list(product_data.keys())
    default_message = (
        f"No se encontraron productos con la presentación: {presentation}."
    )
...
def buscar_productos_por_indicaciones(**kwargs):
    ...
    ids = list(product_data.keys())
    default_message = (
        f"No se encontraron productos con las indicaciones: {indications}."
    )
...
def buscar_productos_por_problema_y_promocion(**kwargs):
    ...
    ids = list(product_data.keys())
    default_message = (
        f"No se encontraron productos en promoción para la consulta sobre: {problem}."
    )

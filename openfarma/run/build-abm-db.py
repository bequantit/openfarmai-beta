...
def get_column_combinations(df):
    ...
    combinations = []
    remaining_columns = [
        col for col in df.columns if col not in BASE_COLUMNS + [ID_COLUMN]
    ]
...
def process_database_combination(df, columns, embedding, name: str = None):
    ...
    if not new_eans:
        print(
            f"Base de datos {db_name} está actualizada. No hay nuevos documentos para agregar."
        )
        return db_name, 0
    ...
    for _, row in df.iterrows():
        ...
        if content_parts:
            ...
            documents.append(Document(metadata={"EAN": ean}, page_content=content))
    ...
    if documents:
        if os.path.exists(persist_dir):
            ...
            print(
                f"Base de datos {db_name} actualizada con {len(documents)} nuevos documentos."
            )
        else:
...
def process_csv_data():
    ...
    db_name, num_docs = process_database_combination(
        df, df.columns.tolist(), embedding, name="db_all"
    )
